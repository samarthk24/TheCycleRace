...
function draw(){
  ...
  if(gameState===START){
    textSize(50);
    fill(255);
    text("Welcome to The Cycle Race!",width/2-300,height/2-50);
    textSize(20);
    text("Dodge the other cyclists or collect shields for protection. Press the up arrow or tap to start racing!", width/2-200, height/2+50, 500, 500);
  ...
  if(shieldCount > 0){
    ...
    fill("white");
    text("Shield duration: " + shieldCount, width - 200,60);
  }
  ...
  if(gameState === END){
    ...
    textSize(20);
    fill(255);
    text("Press Up Arrow or Tap to Restart the game!", width/2-200,height/2+100);
...
function spawnShield() {
  if(World.frameCount%200==0 && shieldCount==0){
    shield = createSprite(width,Math.round(random(50, height-50)));
